...
| <a name="input_enable_guardduty"></a> [enable\_guardduty](#input\_enable\_guardduty) | Within threat-detection, provision the GuardDuty detector. Defaults to true; set to false to skip just GuardDuty (Security Hub and Access Analyzer keep their defaults). Ignored when enable\_threat\_detection = false. | `bool` | `true` | no |
...
| <a name="input_enable_securityhub"></a> [enable\_securityhub](#input\_enable\_securityhub) | Within threat-detection, enable Security Hub. Defaults to true; set to false to skip just Security Hub (GuardDuty and Access Analyzer keep their defaults). Ignored when enable\_threat\_detection = false. | `bool` | `true` | no |
